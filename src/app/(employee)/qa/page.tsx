              ...
              <div className="flex flex-wrap gap-2">
                {suggestions.slice(0, 4).map((s, i) => (
                  <button key={i} onClick={() => sendMessage(s)} disabled={loading}
                    className="rounded-full border border-orange-200 bg-white px-3.5 py-2 text-[13px] font-medium text-orange-700 shadow-sm transition hover:bg-orange-50 active:scale-95 disabled:opacity-50">
                    {s}
                  </button>
                ))}
              </div>
