...
const { data: score }: any = await supabase
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <h1 className="text-2xl font-semibold tracking-tight text-gray-900">Ma note</h1>

      {/* Score principal */}
      <div className="animate-pop-in rounded-xl border border-gray-200 bg-white p-8 text-center shadow-sm">
        <div className="mx-auto flex h-28 w-28 items-center justify-center rounded-full bg-gray-100">
          <span className={`text-4xl font-semibold ${getScoreColor(globalScore)}`}>
            {globalScore}
          </span>
        </div>
        ...
        <div className="mx-auto mt-4 h-3 w-48 sm:w-64 overflow-hidden rounded-full bg-gray-100">
          <div
            className={`h-full rounded-full transition-all ${
              globalScore >= 80 ? "bg-green-500" :
              globalScore >= 60 ? "bg-brand-600" :
              globalScore >= 40 ? "bg-amber-500" : "bg-red-500"
            }`}
            style={{ width: `${globalScore}%` }}
          />
        </div>
        <p className="mt-2 text-xs text-gray-500">Score sur 100</p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div className="animate-fade-in-up rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <p className="text-xs font-medium uppercase text-gray-500">Formation</p>
        ...
        <div className="animate-fade-in-up rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <p className="text-xs font-medium uppercase text-gray-500">Quiz</p>
            <span className="rounded-md bg-orange-50 px-2 py-0.5 text-[10px] font-medium text-brand-700">30% du score</span>
          </div>
          <p className="mt-2 text-2xl font-semibold text-gray-900">{Math.round((score?.quiz_avg_score || 0) * 100)}%</p>
          ...
          <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-gray-100">
            <div className="h-full rounded-full bg-brand-600" style={{ width: `${(score?.quiz_avg_score || 0) * 100}%` }} />
          </div>
        </div>
        ...
        <div className="animate-fade-in-up rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <p className="text-xs font-medium uppercase text-gray-500">Conversations IA</p>
            <span className="rounded-md bg-orange-50 px-2 py-0.5 text-[10px] font-medium text-brand-700">20% du score</span>
          </div>
          <p className="mt-2 text-2xl font-semibold text-gray-900">{score?.conv_hours || 0}h</p>
          <p className="text-xs text-gray-500">
        ...
        <div className="animate-fade-in-up rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <p className="text-xs font-medium uppercase text-gray-500">Q&A Manuel</p>
            <span className="rounded-md bg-orange-50 px-2 py-0.5 text-[10px] font-medium text-brand-700">10% du score</span>
          </div>
